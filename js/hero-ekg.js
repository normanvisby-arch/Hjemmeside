/* ============================================================
   HEDENSTED LÆGEHUS — 3D/WebGL hero til LABORATORIET
...
function init(canvas) {
  ...
  // Laboratoriets hero er lavere og dermed bredere i forhold til højden,
  // så kurven strækkes længere ud til begge sider
  const SAMPLES = 560;
  const RADIAL = 8;
  const X_MIN = -16, X_MAX = 7.2; // kurven udspringer fra "skrivespidsen" i højre side
  ...
  // Radius: tynd hale (venstre) → kraftigt hoved (højre)
  function radiusAt(i) {
    const f = i / (SAMPLES - 1);
  ...
  function updateTrace(time) {
    ...
    //    det nyeste fødes ved skrivespidsen i højre side)
    for (let i = 0; i < SAMPLES; i++) {
      const x = X_MIN + i * DX;
  ...
  // Skrivespidsen sidder i højre ende — signalet strømmer ud fra den mod venstre
